 ...
 */
var shipRespawnTime = () => randFloat(4, 5);

/**
 * Limit of ships on the map when spawning them.
 * Have at least two ships, so that both sides will be visited.
 */
var shipCount = (t, numPlayers) => Math.max(2, Math.round(Math.min(3, t / 5) * numPlayers));

/**
 * Order all ships to ungarrison at the shoreline.
 */
var shipUngarrisonInterval = () => randFloat(3, 4);

/**
 * Total count of gaia attackers per shipload.
 */
var attackersPerShip = t => Math.round(Math.min(30, 10 + t * 2 / 3));
...
/**
 * Ships and land units will queue attack orders for this amount of closest ships.
 */
var targetCount = 3;
...
Trigger.prototype.SpawnAndGarrisonBuilding = function(gaiaEnts, targetClass, templates)
{
	for (let gaiaEnt of gaiaEnts)
	{
		...
		{
			let newEnts = TriggerHelper.SpawnUnits(gaiaEnt, "units/" + template, unitCounts[template], gaulPlayer);
...
Trigger.prototype.CheckShipSunk = function(data)
{
	...
	if (this.ships.indexOf(data.entity) != -1)
	{
		this.debugLog("Ship " + data.entity + " sunk");
		this.ships.splice(this.ships.indexOf(data.entity), 1);
	}

	if (this.ritualEnts.indexOf(data.entity) != -1)
		this.ritualEnts.splice(this.ritualEnts.indexOf(data.entity), 1);
};
 ...
 */
Trigger.prototype.SpawnAndGarrisonShips = function()
{
...
Trigger.prototype.UngarrisonShipsOrder = function()
{
	...
	if (!ungarrisonLeft && !ungarrisonRight)
	{
		...
		cmpTimer.CancelTimer(this.ungarrisonShipsTimer);
		cmpTimer.CancelTimer(this.checkShipRangeTimer);
		return;
	}
	...
	// Order those ships to move to a randomly chosen trigger point on the determined side of the river,
	// remember that chosen ungarrison point and the name of the trigger points where the ungarrisoned units should patrol then
...

{
	...
	cmpTrigger.ungarrisonShipsTimer = cmpTrigger.DoRepeatedly(shipUngarrisonInterval() * 60 * 1000, "UngarrisonShipsOrder", {});
	cmpTrigger.checkShipRangeTimer = cmpTrigger.DoRepeatedly(5 * 1000, "CheckShipRange", {});
	cmpTrigger.DoRepeatedly(shipRespawnTime() * 60 * 1000, "SpawnAndGarrisonShips", {});
