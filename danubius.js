...
const aBlacksmithDecor = "actor|props/structures/hellenes/blacksmith_spears";
...
var randomTreasureCount = 20; //randIntInclusive(0, 3 * numPlayers);
...
if (mapSize >= smallMapSize)
{
	...
	for (let i = 0; i < 2; ++i)
	{
		...
		if (addCelticRitual)
		{
			...
			let femaleRadus = mRadius * 0.3;
			let maleRadus = mRadius * 0.4;
			let benchRadius = mRadius * 0.5;
			let rugRadus = mRadius * 0.6;
			let goatRadus = mRadius * 0.8;

			wallStyles.celt_ritual = {
				"female": new WallElement("female", oFemale, PI, femaleRadus, 0, 2 * PI / femaleCount),
				"skirmisher": new WallElement("skirmisher", oSkirmisher, PI, maleRadus, 0, 2 * PI / maleCount),
				"healer": new WallElement("healer", oHealer, PI, maleRadus, 0, 2 * PI / maleCount),
				"fanatic": new WallElement("fanatic", oNakedFanatic, PI, maleRadus, 0, 2 * PI / maleCount),
				"bench": new WallElement("bench", aBench, PI/2, benchRadius, 0, 2 * PI / benchCount),
				"rug": new WallElement("rug", aRug, 0, rugRadus, 0, 2 * PI / rugCount),
				"goat": new WallElement("goat", oGoat, PI, goatRadus, 0, 2 * PI / goatCount),
			};
...
for (let i = 0; i < numPlayers; ++i)
{
	...
	let painter = new LayeredPainter([tShore, tRoad], [1]);
	...
	let bbDist = 10;
	let bbX = round(fx + bbDist * Math.cos(bbAngle));
	let bbZ = round(fz + bbDist * Math.sin(bbAngle));
	createObjectGroup(
		new SimpleGroup(
	...
	// Create metal mine
	let mDist = scaleByMapSize(9, 14);
	bbAngle += randFloat(PI/4, PI/3);
	let mX = round(fx + mDist * Math.cos(bbAngle));
	let mZ = round(fz + mDist * Math.sin(bbAngle));
	createObjectGroup(
		new SimpleGroup(
	...
	// Create stone mines
	bbAngle += randFloat(PI/3, PI/2);
	mX = round(fx + mDist * Math.cos(bbAngle));
	mZ = round(fz + mDist * Math.sin(bbAngle));
	createObjectGroup(
		new SimpleGroup(
	...
	let num = 20;
	bbAngle += randFloat(-PI/3, PI * 4/3);
	let tDist = randFloat(10, 14);
	let tX = round(fx + tDist * Math.cos(bbAngle));
	let tZ = round(fz + tDist * Math.sin(bbAngle));
	createObjectGroup(
		new SimpleGroup(
